...
<%
    List<CrewDTO> lista = (List<CrewDTO>) request.getAttribute("crew");
    request.setAttribute("pagina", "crew");
%>
...
<div class="container my-5 d-flex justify-content-center">
    <div class="card shadow w-100" style="max-width: 800px;">
        <div class="card-body">
            <h1 class="card-title text-center mb-4">Lista de Crews</h1>

            <form:form method="post" action="/crew/filtrar" modelAttribute="filtro" class="d-flex mb-4">
                <form:select path="tipo" cssClass="form-select me-2" style="max-width: 200px;">
                    <form:option value="nombre" label="Nombre" />
                    <form:option value="puesto" label="Puesto" />
                    <form:option value="empresa" label="Empresa" />
                </form:select>

                <form:input path="nombre" cssClass="form-control me-2" placeholder="Filtrar por nombre..." />
            ...
            <div class="table-responsive">
                <table class="table table-striped align-middle text-center">
                    ...
                    <tbody>
                    <%
                        for(CrewDTO crew : lista){
                    %>
                    <tr>
                        <td><%= crew.getId() %></td>
                        <td>
                            <%=crew.getNombrePersona()%>
                        </td>
                        <td><%= crew.getJob() %></td>
                        <td>
                            <%=crew.getNombreProductCompany()%>
                        </td>
                        <td colspan="2">
            ...
            <form method="post" onsubmit="abrirModal(this, 'editar'); return false" class="text-center mt-4">
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="bi bi-plus-circle me-2"></i>Crear nuevo crew
                </button>
            </form>
        </div>
    </div>
        ...
        if (tipo === "editar") {
            fetch("/crew/editar", {
                ...
                .then(html => {
                    ...
                    // Ahora asignamos la validación al formulario del modal cargado
                    const form = modalEl.querySelector("form");
                    if (form) {
                        form.onsubmit = function (e) {
                            const persona = document.getElementById("idPerson").value.trim();
                            if (persona === "") {
                                alert("Debes seleccionar una persona.");
                                e.preventDefault();  // Previene el envío del formulario si no hay persona seleccionada
                                return false;
                            }
                            return true;
                        };
